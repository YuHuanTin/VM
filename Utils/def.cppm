...
export {
    ...
    void CHECK_ERR(auto ReturnValue, std::source_location Location = std::source_location::current()) {
        if (ReturnValue != UC_ERR_OK) {
            throw std::runtime_error(std::format("Failed on `{}` with uc error returned: {}\n{}+{}:{} ", Location.function_name(), static_cast<unsigned int>(ReturnValue), Location.file_name(), Location.line(), Location.column()));
        }
    }

    inline std::string ReadFileBinary(const std::string_view &FilePath, const bool CreateIfNotExist = false) {
        std::fstream fs(FilePath.data(), std::ios_base::in | std::ios_base::binary);
