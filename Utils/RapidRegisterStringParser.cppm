...
export module RapidRegisterStringParser;
    ...
    inline REGS ParseRegisterString(const std::string &Str) {
        ...
        while (std::getline(ss, newLine, '\n')) {
            ...
            if (registerName == "RAX") {
                ...
                // check TF
                if ((value & 1 << 8) != 0) {
                    std::println("Warning: Trap Flag is set in RFLAGS, which may cause single-step exceptions during emulation. Consider clearing it for better performance.");
                }
                regs.rflags_ = value;
                break;
            }
    ...
    inline REGS_X86 ParseRegisterString_X86(const std::string &Str) {
        ...
        while (std::getline(ss, newLine, '\n')) {
            ...
            if (registerName == "EAX") {
                ...
                // check TF
                if ((value & 1 << 8) != 0) {
                    std::println("Warning: Trap Flag is set in EFLAGS, which may cause single-step exceptions during emulation. Consider clearing it for better performance.");
                }
                regs.eflags_ = value;
                break;
            }
